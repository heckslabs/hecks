...
# `rspec_shard` leg's wall-clock (measured once at 75.7s on one worker,
...
module QaMineCombinationsHelpers
  ...
  # Runs `bin/qa_mine_combinations` as a real subprocess against `CORPUS`,
  # with the agent stubbed by the `spec/fixtures/qa_mine_combinations`
  # fake agent.
  #
  # @param mode [String] the fake agent's mode, via `FAKE_AGENT_MODE`
  #   ("valid", "silent", "repair", …)
  # @return [Array(String, Process::Status)] the subprocess's combined
