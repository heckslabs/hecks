...
# "postgres").close` round trip is real I/O, and without this shared
# probe, five independent copies of it would run on every `bundle exec
# rspec`, all asking the exact same question. `@available`'s own
# memoization is what makes "ONCE" true — every caller below reaches
# this same method.
#
# ## Lazy on purpose
#
# `.available?` must be a method, not a constant computed
...
# the suite: without this, a CI job whose Postgres fails to come up would
# go green having run none of them. CI provisions Postgres for every job
# that runs a spec reaching this probe (the `rspec_postgres_io*`,
# `rspec_rust_host`, `rspec_fuzzing` and `stress_concurrency` jobs); the
# light `rspec` shard runs `--tag ~io` and never reaches it.
# `rspec_fuzzing` is the example of why this matters: it had no Postgres,
# so its two io fuzzing specs skipped there — and every other io job runs
# `--tag ~fuzzing`, so they ran nowhere. So under `ENV["CI"]` a `false`
# answer raises instead — the whole group fails, naming the connection
# error. Locally (no `CI`) it still answers `false` and the spec skips.
#
# ## Requiring `pg` explicitly
#
