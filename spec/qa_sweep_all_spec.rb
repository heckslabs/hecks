...
RSpec.describe "bin/qa_sweep --all", :io do
  ...
  #
  # REAPED BY `Process.waitpid2(pid, Process::WNOHANG)`, POLLED — NOT
  # `process_alive?`, and this is load-bearing, not a style choice.
  # `process_alive?` is `Process.kill(0, pid)` rescuing `ESRCH`, and a
  # ZOMBIE (already exited, not yet reaped) still answers signal 0
  # successfully — a zombie IS a process-table entry, just an exited
  # one. A `while process_alive?(pid)` loop can therefore never observe
  # this "--all" subprocess exiting: the instant it does, it becomes
  # exactly the zombie `process_alive?` cannot tell apart from "still
  # running," and nothing in that loop would EVER call the one thing
  # that reaps it (`Process.waitpid2`, only reachable AFTER the loop
  # exits) — a guaranteed, not merely occasional, hang the moment the
  # child actually finishes. Confirmed live: a real run hung for over an
  # hour this way, a zombie `<defunct>` `bin/qa_sweep --all` sitting as
  # this exact test's own `pid`, its parent's `Process.kill(0, ...)`
  # polling loop cheerfully reporting it alive forever. `process_alive?`
  # itself is left as-is — the file's OTHER use of it (two lines above,
  # a one-shot check immediately followed by a real blocking
  # `Process.waitpid2`, never a loop condition) never depends on telling
  # a zombie apart from a live process, so it never had this bug.
  # `WNOHANG` is the correct shape for "have I finished yet, without
  # blocking" — it reaps AND asks in the same call, so there is no
  # separate window where an exited child sits unreaped waiting on this
  # loop's next iteration to notice.
  # THE NON-BLOCKING REAP LOOP ITSELF — pulled out of the example so the
  # example reads as "spawn, watch, assert" rather than burying the
  # `WNOHANG` mechanics inline. `probe` is called once per poll and
  # returns whatever this run wants tracked (here, the live `pool_`
  # count); returns `[status, probe_results]`.
  ...
  # THE FAILURE MODE THAT PROMPTED THE COMMENT ABOVE, EXERCISED DIRECTLY
  # — not just the happy path where children happen to take a couple of
  # seconds each. Every target here finishes about as fast as a real OS
  # process CAN (`--seeds 1 --steps 1` against the same trivial Widget
  # fixture, no Rust build, no invariant a fuzzer could ever trip), so
  ...
  # exits very early/fast" shape a bounded pool's bookkeeping has to
  # survive. `run_qa_sweep` blocks on `Open3.capture3` (which reaps its
  # own child internally, no separate zombie-detection step to get
  # wrong), so this proves `run_pool` itself never stalls waiting for a
  # slot that is already free, independent of the OTHER test's own
  # now-fixed polling bug above.
