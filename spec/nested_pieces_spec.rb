...
# this corpus had never actually reached before. BUG#3 was an addressing identity that failed
...
RSpec.describe "NestedPieces" do
  ...
  # BUG#12 — an entity created via `sets :list, append: {...}` leaves a
  # declared attribute the append mapping doesn't name (`Board
  # .label`, `Card.note`, both `optional: true`) present in the stored
  # hash, at both nesting depths this domain exercises: the
  ...
  # — nested two levels deep, a shape once called "out
  ...
  # **Hop one** — the same single-level shape BUG#3 first surfaced on
  ...
  # ever consulted) — is immune to BUG#3 independent of the fix,
