...
# machine, not a bare attribute — and hardcoding `record.state[:state]` in
# `Comparison#none_in_state?` would silently read `nil` off any
...
RSpec.describe "none_in_state against a lifecycle-backed target" do
  ...
  it "reads the target's own declared lifecycle field, not a hardcoded :state key" do
    ...
    # Hardcoding `record.state[:state]` would read `nil` for every row
