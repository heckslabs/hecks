...
RSpec.describe "the DSL surface" do
  ...
  def build_command(domain, &block)
    build_aggregate(domain) do
      ...
      # happen silently, while still asserting the mutation had been recorded.
  ...
  describe "declarations that cannot mean what they say" do
    ...
    # the aggregate and never the piece. This is not "a
    # reference to an entity rather than an aggregate head" — that is not what
    ...
    # object through `closed_sets`, not `@value_objects`, and would be
    # invisible to this exact check without also reading `closed_sets`:
    # `attribute :cover, one_of("covered",
    # "open"), default: "open"` would build cleanly and then refuse every
    # create at dispatch, the same silent-then-loud failure the
    # block-declared case above is already sealed against.
    # A name declared twice would otherwise survive into the IR as two distinct
    ...
    # #[]`) would silently see only the first, the second permanently
    ...
    # L7 (docs/audits/2026-08-11-bug-triage.md, Tier 7) — calling `.to_s` on
    # every member field value in `to_h`'s own `members:` emission would mean
    # `minor_units: 2` (a genuine Integer, `currency.members` above
    # proves it) crosses the wire as the string `"2"` — indistinguishable
  ...
  describe "a bluebook" do
    ...
    it "gathers includes declared before the reference, in either order" do
      # `many:` is decided by comparing each include against the reference. The
      # includes are resolved at build, which removes the ordering rule rather
      # than moving it.
    ...
    # "declares no correlates_by"). Spelling that absent case
    # as `""` in `to_h` would be indistinguishable on the wire from a genuinely empty name,
    # and would read back as the wrong non-nil `:""` rather than `nil`.
  ...
  describe "an aggregate" do
    ...
    it "lifecycle refuses to guess a target when no declared from: admits the current state" do
      ...
      # "z" admits neither declared "Advance" transition — silently falling
      # back to the first one ("b") would be a wrong answer for a
    ...
    # Without it, every case here would build cleanly and answer wrongly forever: a
