...
# transition matcher: a VO-typed lifecycle field's current state is
# unwrapped via `Value.scalar` before `.to_s`, so `current` reads the
# inner scalar rather than hitting Ruby's default Object#to_s on the
# wrapping Value, which would come back as a raw object-pointer string
# that could never match any declared `from` state. Bites on the second
