...
# diverge on two counts unless a real `projected_by` binding actually
# routes a read model through the native path (nothing in the
...
#   the native path would answer a silent `{root: nil, ...}`.
...
#   path would match only against the root, so a chained head's own
#   rows would come back empty no matter what actually existed.
