    ...
    module Deploy
      ...
      module Lambda
        ...
        def call(bluebook:, options: {})
          ...
          #
          # Empty when `shared` — this domain provisions no VPC/RDS of its own to
          # expose at all (bastion.yaml/bastion_parameters/mint-era below are all
          # skipped for the same reason: there's nothing here for a bastion to
          # reach). `shared`'s own stack is never an owner, so nothing downstream
          # ever needs to look these up from it either.
          stack_outputs = (shared ? [] : [
            ...
            # a "Shared"-mode domain elsewhere (bin/project_deploy's own Shared
            # branch, above) looks these two up live to attach its own Lambda to
            # this domain's VPC/security group without minting either itself,
            # the same reuse #{web_logical_id} already does for
            # #{logical_id}SecurityGroup, one stack over instead of one resource
            # over.
            { key: "FunctionSecurityGroupId", var: "FN_SG_ID",            ref: "!Ref #{logical_id}SecurityGroup" },
          ...
          # that can silently drift between the two files without this shared table.
          #
          # Empty when `shared` — no bastion.yaml gets generated at all (below);
          # era-minting for a Shared-mode domain reuses its owner's already-live
          # tunnel instead of standing up a temporary bastion of its own next to
          # an RDS instance that, in Shared mode, doesn't exist. See the Makefile
          # section's own comment on `mint-era` for the manual path this leaves.
          bastion_parameters = (shared ? [] : [
          ...
          template_yaml = <<~YAML
            ...
            Resources:
              ...
              #{if aurora
                  ...
                  # plain-RDS path below; Adapters::PostgresEra/tokio_postgres speak
          ...
          unless shared
            ...
            # #{infra_name}'s private RDS instance and mint era 1 (see
        ...
        # The actual YAML — built as one plain string here, not inline inside
        # the big template heredoc below, so the doc comment explaining it can
        # use ordinary `#{...}` Ruby interpolation without a second layer of
        # escaping fighting the template's own. Empty string (contributes
        # nothing) for the common case, a domain with no cross-domain policies
        # at all.
        # Every line hand-indented to its own final column, not built through
        # `<<~`'s squiggly-heredoc dedent + a second blanket-prefix pass —
        # stacking those two independently-computed indentation adjustments is
        # exactly what produced a real, caught-by-hand bug the first time this
        # was written (the `Resource:` list landing 8 columns deeper than every
        # sibling key, from the dedent's own remainder adding to the blanket
        # prefix instead of being replaced by it). One explicit column per line
        # is more to type, never ambiguous about what it renders as.
        # A function of the marker's own actual indentation, not a hardcoded
        # column count — found live: the enclosing `<<~YAML` heredoc (below)
        # dedents by its own computed minimum across every line, which shifts
        # whenever the surrounding template's own baseline indentation shifts
        # (6, here — not the 8 a naive reading of this block's own source
        # indentation would suggest); hardcoding a column guess drifts out of
        # sync with that the next time anything upstream in the same heredoc
        # changes its own minimum. `base` is whatever whitespace the "# tmpl:
        # cross_domain_lambda_policies" marker line actually rendered at,
        # captured by regex after the heredoc renders (below) — every line here
        # is built relative to it.
        #
        # @param targets [Array<String>] the domain names this stack's `across:` targets declare
        # @param base [String] the marker line's own rendered indentation whitespace, everything
        #   below is built relative to it
        # @return [String] the rendered comment plus one `LambdaInvokePolicy`-shaped IAM statement
        #   granting invoke on each target's function ARN, ending in exactly one trailing newline;
        #   an empty string if `targets` is empty
        def cross_domain_lambda_policies_yaml(targets, base)
          return "" if targets.empty?

          comment = [
            "LEAST-PRIVILEGE, one ARN per declared `across:` target — the SAME",
            "`LambdaInvokePolicy`-shaped grant WebFunctionRole's own Policies",
            "section (below) exists for, extended to a target this stack does",
            "not own (so no `!Ref FunctionName` to reach for — the target's",
            "own function name is `lambda_client.rs`'s own computed",
            '"hecks-#{domain}" convention, read directly, matching every',
            "OTHER function-name computation in this whole project). Declared,",
            "not deployed: an ARN naming a function that does not exist YET is",
            "still valid IAM policy — the call simply fails with",
            "ResourceNotFoundException instead of AccessDeniedException until",
            "the target domain is deployed for real. Without this grant, EVERY",
            "cross-domain policy match this domain ever fires refuses with a",
            "hard invoke fault (AccessDeniedException) regardless of what the",
            "target itself would have answered — found live, standing up a",
            "real second domain (examples/compliance) specifically to prove",
            "cross-domain delivery end to end for the first time; nothing",
            "before this ever granted this domain's own Lambda role permission",
            "to invoke ANY other function at all. Targets: #{targets.join(', ')}.",
          ...
          # own default) — `String#sub` (below) replaces the marker line whole,
