module Hecks
  module Bluebook
    ...
    # reads it, and the coverage gate holds the table to the language. A
    # method-per-category shape costs fourteen verbs the language declared and
    # nothing ever offered — the price this file's own first draft paid before
    # moving to a shared table.
    #
    ...
    class Assembly
      # Builds a whole bluebook graph from its plain declaration.
      #
      # @param declaration [Hash{Symbol => Object}] the chapter's declaration, the same
      #   shape a `to_h` (or `Reconstruction`) produces
      # @return [Bluebook::Chapter] the assembled, owner-stamped chapter
      ...
      # Assembles this instance's declaration into a whole bluebook graph.
      #
      # @return [Bluebook::Chapter] the assembled, owner-stamped chapter
      def bluebook
