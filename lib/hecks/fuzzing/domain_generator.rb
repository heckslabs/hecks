...
module Hecks
  module Fuzzing
    ...
    #
    # ## A blueprint, not source
    #
    # `generate` answers a plain,
    ...
    module DomainGenerator
      ...
      # Builds and prunes one blueprint from a seed.
      #
      # @param seed [Integer] RNG seed; every draw this generation makes is
      #   reproducible from it
      # @param forms [Array<String, Symbol>, nil] the two `FORMS` to force onto the
      #   primary aggregate; `nil` draws two at random from `seed`
      # @return [Hash] the generated, pruned blueprint, with `"seed"` and `"forms"`
      #   merged in
      # @raise [ArgumentError] if `forms` names anything not in `FORMS`
      ...
      #
      # @param blueprint [Hash] the blueprint to write, as returned by `#generate`
      #   (or carrying `"source"` for an adopted domain)
      # @param root [String] the directory to write `qa_generated/` and
      #   `blueprint.json` under
      # @return [String] path to the written domain directory
      ...
      #
      # @param source [String] the bluebook source text to rename
      # @return [String] `source` with its `Hecks.bluebook "..."` chapter name
      #   rewritten to `QaGenerated`
      def adopt(source)
      ...
      # Renders a full blueprint as bluebook source text.
      #
      # @param blueprint [Hash] the blueprint to render, as returned by `#generate`
      # @return [String] the rendered bluebook source, newline-terminated
      ...
      # Renders one aggregate as bluebook source lines.
      #
      # @param aggregate [Hash] a blueprint aggregate fragment
      # @return [Array<String>] the aggregate's own rendered source lines
      ...
      # Renders an aggregate's own opening lines.
      #
      # @param aggregate [Hash] a blueprint aggregate fragment
      # @return [Array<String>] the `aggregate "Name" do ... identified_by ...`
      #   opening lines
      ...
      # Renders one entity as bluebook source lines.
      #
      # @param entity [Hash] a blueprint entity fragment
      # @return [Array<String>] the entity's own rendered source lines
      ...
      # Renders one attribute as a bluebook source line.
      #
      # @param attribute [Hash] a blueprint attribute fragment
      # @return [String] the attribute's own rendered `attribute :name, Type, ...`
      #   source line
      ...
      # Renders one value object as bluebook source lines.
      #
      # @param name [String] the value object's own declared name
      # @param value_object [Hash] a blueprint value object fragment
      # @param indent [String] leading whitespace for each rendered line
      # @return [Array<String>] the value object's own rendered source lines
      ...
      # Renders one lifecycle as bluebook source lines.
      #
      # @param lifecycle [Hash] a blueprint lifecycle fragment
      # @param indent [String] leading whitespace for each rendered line
      # @return [Array<String>] the lifecycle's own rendered source lines
      ...
      # Renders one command as bluebook source lines.
      #
      # @param command [Hash] a blueprint command fragment
      # @param self_name [String, nil] the owning aggregate's own name, to render
      #   an implicit `reference_to` for a non-creating entity command; `nil` skips it
      # @param indent [String] leading whitespace for each rendered line
      # @return [Array<String>] the command's own rendered source lines
      ...
      # Renders one `sets` mutation as a bluebook source line.
      #
      # @param set [Hash] a blueprint `sets` mutation fragment
      # @return [String] the mutation's own rendered `sets :target, ...` source line
      def render_set(set)
      ...
      # Renders one query as bluebook source lines.
      #
      # @param query [Hash] a blueprint query fragment
      # @return [Array<String>] the query's own rendered source lines
      ...
      # Renders one policy as bluebook source lines.
      #
      # @param policy [Hash] a blueprint policy fragment
      # @return [Array<String>] the policy's own rendered source lines
      ...
      # @return [Array<Hash>] every blueprint one removal smaller, each already
      #   pruned and deduplicated; `blueprint` itself is never included
      ...
      # Every removable element's own path into the blueprint.
      #
      # @param blueprint [Hash] the blueprint to enumerate removable paths in
      # @return [Array<Array(String, Integer)>] one path per removable element
      #   (a policy, an aggregate past the first, a query/invariant/entity/
      #   reference/attribute/command, or a command's own given/role/emit)
      ...
      # Every removable path under an aggregate's own entities.
      #
      # @param aggregate [Hash] a blueprint aggregate fragment
      # @param at [Array(String, Integer)] `aggregate`'s own path in the blueprint
      # @return [Array<Array(String, Integer)>] paths to each of `aggregate`'s own
      #   entities' removable lifecycle and commands
      ...
      # Every removable path under one command.
      #
      # @param command [Hash] a blueprint command fragment
      # @param at [Array(String, Integer)] `command`'s own path in the blueprint
      # @return [Array<Array(String, Integer)>] the command's own removable path
      #   (unless it creates its aggregate), plus its own givens, role, and a
      #   redundant emit
      ...
      # Removes the element at one path, on a deep copy.
      #
      # @param blueprint [Hash] the blueprint to remove from
      # @param path [Array(String, Integer)] the path to remove, as returned by
      #   `#removals`
      # @return [Hash] a deep copy of `blueprint` with the element at `path` removed
      ...
      # @param blueprint [Hash] the blueprint to prune
      # @return [Hash] a deep copy of `blueprint` with every dangling element
      #   dropped, to a fixpoint
      def prune(blueprint)
      ...
      # One pruning pass over `blueprint`, in place — not necessarily a fixpoint;
      # see `#prune`'s own loop.
      #
      # @param blueprint [Hash] the blueprint to prune, mutated in place
      # @param available [Set<String>] the tokens still present, as returned by
      #   `#tokens`
      # @return [void]
      ...
      # Drops one aggregate's own dangling references, attributes, invariants,
      # entities, queries, lifecycle, and command fragments.
      #
      # @param aggregate [Hash] a blueprint aggregate fragment, mutated in place
      # @param available [Set<String>] the tokens still present
      # @param keep [Proc] answers whether one element's own `"requires"` tokens
      ...
      # Drops a lifecycle's own dangling transitions, and the lifecycle itself
      # if nothing survives.
      #
      # @param owner [Hash] a blueprint aggregate or entity fragment, mutated in
      #   place
      # @param available [Set<String>] the tokens still present
      # @return [void]
      ...
      # goes too: without this, shrinking away `Close` would leave `Reopen
      # from closed` behind — qa/stress_domains/generated_revalued_shape
      # was promoted that way, and bin/model_check reports it as a dead
      # transition.
      #
      # @param default [String] the lifecycle's own default state
      # @param transitions [Array<Hash>] the lifecycle's own transition fragments,
      #   each with `"from"` (`Array<String>`) and `"to"` (`String`)
      # @return [Array<String>] every state reachable from `default`, `default`
      #   included
      ...
      # Drops a command's own dangling reference, args, givens, and sets.
      #
      # @param command [Hash] a blueprint command fragment, mutated in place
      # @param available [Set<String>] the tokens still present
      # @return [void]
      ...
      # Every token a blueprint's own elements currently declare, for `requires`
      # to be checked against.
      #
      # @param blueprint [Hash] the blueprint to collect tokens from
      # @return [Set<String>] every declared aggregate/lifecycle/attribute/
      #   reference/command/command_reference/event/entity token, tagged by kind
      ...
      #
      # @param name [String] a PascalCase name
      ...
      class Builder
        # @param random [Random] the seeded RNG every draw this build makes comes
        #   from
        # @param forms [Array<String>] the two `FORMS` to force onto the primary
        #   aggregate
        ...
        # Runs the seeded pass this instance was configured for.
        #
        # @return [Hash] `{"aggregates" => Array<Hash>, "policies" => Array<Hash>}`,
        #   the built (not yet pruned) blueprint fragment
        def build
