  ...
  module Runtime
    class CommandInterpreter
      ...
      module ArgumentGate
        ...
        # Without this refusal, anything else would ride along in the payload
        # untouched — normalize_args walks the declared attributes, so a
        # name the command never had would simply never be looked at, and a
        ...
        #
        # Without this refusal, `Customer.Register` missing its `name` would
        # still be refused — but by accident, and with a lie for a message:
        # `then_set :name, to: :name` finds nothing to resolve, passes the
        # literal symbol on, and coercion reports `name is a PersonName —
        # pass its fields as an object`, which describes a mistake the
        # caller did not make. The real mistake — an argument simply
        # missing — is never the one named, and nothing refuses it on
        # purpose without this check.
        #
        ...
        # has no other way to declare which record it addresses. `to:` is
        # that other way (`Invocation.port_call` promotes the identity
        # attribute's own value out of the payload and into routing) —
        # which leaves the attribute still declared, still non-optional,
        # and never present in `args` at all: every operation with an
        # identity attribute would otherwise refuse its own well-formed
        # calls, dispatched exactly the way `to:` intends. Exempted here on
        # the same terms an aggregate command's self-address always is —
        ...
        # **Declaring nothing still must say so**. `Account
