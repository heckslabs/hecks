                ...
                if let Some((id_attr, id_vo)) = entity_identity_mint(entity, value_objects_by_name) {
                    ...
                    if !present.iter().any(|p| p == id_name) {
                        ...
                        collision_guard = format!(
                            "if record.{target_field}.iter().any(|e| e.{id_field} == {id_rhs}) {{ return Err(crate::kernel::Refusal::AlreadyExists(crate::kernel::RefusalSite::AlreadyExistsEntityDuplicate.render(&[(\"entity\", {entity_lit}), (\"aggregate\", {aggregate_lit}), (\"identity\", {identity_lit}), (\"offered\", &format!(\"{{:?}}\", {offered_expr}))]))); }}\n        "
                        );
                    }
                }
