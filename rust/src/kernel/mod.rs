...
// `vocab` — the language's Vocabulary tables (refusal templates, query
// comparators, field hints, aggregate/entity dispatch orders), one
// exhaustive enum per table, generated by bin/project_rust_vocabulary from
// lib/hecks/language/bluebook/vocabulary.bluebook.
pub mod vocab;
// `refusal_wording` — `RefusalSite`, kept at its old path: the module is
// now `vocab::refusal_template`, re-exported under the name every existing
// call site (`super::refusal_wording::RefusalSite`) already uses.
