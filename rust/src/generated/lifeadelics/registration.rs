...
if !unknown.is_empty() {
    return Err(crate::kernel::Refusal::UnknownArgument(format!(
        "RegistrationId does not declare {} — it takes value",
        unknown.join(", ")
    )));
}
        Ok(Self {
...
if !unknown.is_empty() {
    return Err(crate::kernel::Refusal::UnknownArgument(format!(
        "Attendee does not declare {} — it takes name, email",
        unknown.join(", ")
    )));
}
        Ok(Self {
...
if !unknown.is_empty() {
    return Err(crate::kernel::Refusal::UnknownArgument(format!(
        "Request does not declare {} — it takes event_slug, registration_id, attendee",
        unknown.join(", ")
    )));
}
