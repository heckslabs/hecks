...
module RustProjection
  ...
  module DomainGenerator
    ...
    def call(ir, source_label, mod_dir, mod_name)
      ...
      # ALSO REQUIRES merged.rs TO STILL EXIST ON DISK, not just that
      # mod.rs still mentions it — a chapter that was a TARGET domain in
      # an earlier run (getting its own merged.rs written by bin/
      # project_rust's own separate step, below) but is only an attached
      # FRAMEWORK chapter this run (never the target) never gets merged.rs
      # regenerated at all; without this check, mod.rs kept claiming
      # "pub mod merged;" from the stale trailer while WriteIfChanged's
      # own pop_and_prune (this file's own header) correctly deleted the
      # orphaned merged.rs it never saw touched this run — a dangling
      # `pub mod merged;` with no file behind it, E0583 ("file not found
      # for module `merged`") on the very next build. Found live
      # regenerating lifeadelics/ after an earlier run had targeted
      # Compliance directly.
      merged_trailer = File.exist?(mod_path) && File.read(mod_path).include?("pub mod merged;") && File.exist?(File.join(mod_dir, "merged.rs"))
