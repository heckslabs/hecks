...
module RustProjection
  ...
  module DomainGenerator
    ...
    # `metadata.rs`, `mod.rs`) into `mod_dir`.
    #
    # @param ir [Hash] the chapter's JSON-shaped IR
    # @param source_label [String] provenance text written into each file header
    # @param mod_dir [String] the chapter's output directory
    # @param mod_name [String] the chapter's lowercase Rust module name
    # @param merged_module [Boolean] whether the caller writes a `merged.rs` beside
    #   this chapter. A framework chapter attached to a target domain never gets one
    #   (its directory is pruned of any `merged.rs` an earlier run left), so its
    #   `mod.rs` must not declare `pub mod merged;`.
    # @return [Hash] the chapter's `:aggregates`, `:queries` and `:read_models`
    # @raise [RuntimeError] if a domain or aggregate name can't be a Rust module name
    def call(ir, source_label, mod_dir, mod_name, merged_module: true)
      ...
      # The trailer also requires the caller to be writing a merged.rs this
      # run (`merged_module`), not just that mod.rs still mentions one. A
      # framework chapter never gets a merged.rs regenerated, and
      # `pop_and_prune` deletes any orphaned one once the chapter's tracked
      # directory closes. That prune runs after this mod.rs is written, so
      # the file's presence on disk here says nothing about whether it
      # survives; keeping the trailer on that basis leaves a dangling
      # `pub mod merged;` (E0583) in the chapter's mod.rs.
